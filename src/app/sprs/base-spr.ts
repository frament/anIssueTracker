...
export class BaseSpr {
  ...
  constructor(name: string, options?: sprOptions){
    this.name = name;
    this.options = { storeLocal: true, index : 'objectId'};
  ...
  add(item: any[]): void{
    if (Array.isArray(item)){
      const indexedItems = item.map((si, i) => {
  ...
  private _getItemArrIndexById(id: string|number){
    const idx = this.items
      .map((x: any, i: number) => ({id: x[this.options.index], idx: i}))
      .filter(x => x.id == id)[0];
    ...
    if (!query){
      ...
      if (query.value instanceof Date){
        ...
        return this.items.filter(x => {
          ...
          for (const key in query.value) if (x[key] != query.value[key]) return false;
          return true;
        });
      }else{
